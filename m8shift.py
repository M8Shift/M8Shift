...
def scaffold_write_gate(confirm_script_dir=False, script_dir=None, command="init"):
    """Refuse ambiguous scaffold writes before their first mkdir/open.

    Bootstrap commands always write beside the installed kit, not beside the
    caller's current directory.  A different cwd therefore needs an explicit
    confirmation.  A conflicting M8SHIFT_ROOT remains a hard refusal because it
    would split script-local scaffold files from the rebased relay state.
    """
    kit_root = os.path.realpath(script_dir or HERE)
    cwd_root = os.path.realpath(os.getcwd())
    env_raw = (os.environ.get("M8SHIFT_ROOT") or "").strip()
    if env_raw:
        env_root = os.path.realpath(os.path.abspath(os.path.expanduser(env_raw)))
        if env_root != kit_root:
            sys.exit(
                "refused: %s is a script-local bootstrap, but M8SHIFT_ROOT resolves "
                "to a different root (%s). It writes beside the kit at %s, not into "
                "the rebased root. Unset M8SHIFT_ROOT, then copy/run the kit from the "
                "exact project root (for example ~/code/My Project)."
                % (command, env_root, kit_root)
            )
    if cwd_root != kit_root and not confirm_script_dir:
        sys.exit(
            "refused: %s writes beside the kit at %s, but the physical current "
            "directory is %s. Copy/run the kit from the exact project root (for "
            "example ~/code/My Project), or explicitly confirm the script-local "
            "target with --confirm-script-dir."
            % (command, kit_root, cwd_root)
        )
    return kit_root
...
TTL_MIN = 30
VERSION = "3.65.0"       # m8shift.py script version (bump on release). Surfaced by `--version`,
...
BOOTSTRAP_BEGIN = "<!-- M8SHIFT:BOOTSTRAP:BEGIN (generated by m8shift.py init - do not edit by hand) -->"
BOOTSTRAP_END = "<!-- M8SHIFT:BOOTSTRAP:END -->"
...
def render_bootstrap_runbook(profile, ids, project_name=None, roster=None):
    """Render the re-entrant, generated recovery region of BOOTSTRAP.md."""
    kit = _read_kit_manifest(HERE)
    installed = {
        row.get("name") for row in _kit_list(kit, "companions")
        if isinstance(row, dict) and isinstance(row.get("name"), str)
    }
    project = project_name or os.path.basename(HERE) or "My Project"
    agents = list(roster or ())
    agent = agents[0] if agents else "agent-a"
    lines = [
        BOOTSTRAP_BEGIN,
        "# Re-entrant M8Shift bootstrap and recovery runbook",
        "",
        "Project: `%s`  " % project,
        "Roster: %s  " % (", ".join("`%s`" % value for value in agents) or "read from `M8SHIFT.agent-pack.md`"),
        "Init profile: `%s`  " % profile,
        "Rendered capabilities: %s" % (", ".join("`%s`" % cap for cap in ids) or "none"),
        "",
        "This runbook is safe to repeat. Reconstruct state from `M8SHIFT.md` (the journal), "
        "`M8SHIFT.agent-pack.md`, `M8SHIFT.protocol.md`, and `.m8shift/kit.json` when that "
        "manifest exists. The manifest's `core.version`, reconciled by `update`, is "
        "authoritative; do not trust the init-time `bootstrap.json.engine_version` after an update.",
        "",
        "## 1. Establish the exact root and versions",
        "",
        "Run from the directory containing this kit and confirm the physical directory before writes:",
        "",
        "```sh",
        "pwd -P",
        "./m8shift.py --version",
        "./m8shift.py status --json",
        "test ! -f .m8shift/kit.json || python3 -m json.tool .m8shift/kit.json",
        "```",
        "",
        "Read `M8SHIFT.agent-pack.md` and `M8SHIFT.protocol.md`, then run the read-only core diagnostic:",
        "",
        "```sh",
        "./m8shift.py doctor",
        "```",
        "",
        "## 2. Runtime, usage, provider, and listener",
        "",
    ]
    runtime_installed = "runtime" in installed
    if runtime_installed:
        lines.extend([
            "The `runtime` companion is recorded in `.m8shift/kit.json`; these steps are enabled:",
            "",
            "```sh",
            "./m8shift-runtime.py doctor",
            "./m8shift-runtime.py usage init",
            "./m8shift-runtime.py usage adapters check",
            "./m8shift-runtime.py usage snapshot",
            "./m8shift-runtime.py providers check %s" % agent,
            "./m8shift-runtime.py providers show %s" % agent,
            "./m8shift-runtime.py listener start --agent %s --provider" % agent,
            "./m8shift-runtime.py listener status --agent %s --json" % agent,
            "```",
            "",
            "`usage init` is deliberately no-clobber. Listener start performs the runner "
            "capability handshake before launch; status is the post-start proof.",
        ])
    else:
        lines.extend([
            "The `runtime` companion is absent from `.m8shift/kit.json`. Install it before "
            "running usage, provider, or listener commands:",
            "",
            "```sh",
            "./m8shift.py init --companions runtime --companion-source <release-dir>",
            "```",
        ])
    lines.extend(["", "## 3. Dashboard", ""])
    if "top" in installed:
        lines.extend([
            "The `top` companion is recorded in `.m8shift/kit.json`:",
            "",
            "```sh",
            "./m8shift-top.py",
            "```",
        ...
        lines.extend([
            "The `top` companion is absent. Install it before launching the dashboard:",
            "",
            "```sh",
            "./m8shift.py init --with-top --companion-source <release-dir>",
            "```",
        ])
    lines.extend(["", "## 4. Recover a halted listener", ""])
    if runtime_installed:
        lines.extend([
            "First inspect the read-only JSON and its additive `cause` field. Preserve that "
            "evidence, inspect bounded logs, repair the reported cause, and only then restart:",
            "",
            "```sh",
            "./m8shift-runtime.py listener status --agent %s --json" % agent,
            "./m8shift-runtime.py listener logs --agent %s --tail 50" % agent,
        ...
        lines.extend([
            "Install the absent `runtime` companion with the command in section 2, "
            "re-run init to refresh this generated block, then follow the enabled "
            "halted-listener recovery commands and inspect their `cause` field.",
        ])
    lines.extend(["", BOOTSTRAP_END])
    return "\n".join(lines)


def ensure_bootstrap_runbook(block):
    """Replace only the unique generated block; preserve all operator prose."""
    path = os.path.join(HERE, ".m8shift", "BOOTSTRAP.md")
    if not os.path.exists(path):
        write(block + "\n", path)
        return ".m8shift/BOOTSTRAP.md: written"
    cur = read(path)
    ...
    if new == cur:
        return ".m8shift/BOOTSTRAP.md: kept"
    write(new, path)
    return ".m8shift/BOOTSTRAP.md: written"


def apply_init_capabilities(ids, profile, project_name=None, roster=None,
                            confirm_script_dir=False):
    scaffold_write_gate(confirm_script_dir, command="init capability scaffold")
    rows = []
    bodies = {
    ...
    rows.append(ensure_bootstrap_runbook(
        render_bootstrap_runbook(profile, ids, project_name, roster)))
    return rows


def ensure_gitignore_block(confirm_script_dir=False):
    scaffold_write_gate(confirm_script_dir, command="gitignore scaffold")
...
def apply_companions(plan, confirm_script_dir=False):
    ...
    scaffold_write_gate(confirm_script_dir, command="companion scaffold")
...
def _update_companions_component(selection, source_dir, checksums, allow_downgrade,
    ...
    if guard is not None and verified:
        guard.require_owned()
    lines, apply_errors = apply_companions(verified, confirm_script_dir=True)
    notes.extend(lines)
    for entry in verified:
...
def cmd_init(args):
    if getattr(args, "list_profiles", False):
        ...
        return 0
    confirm_script_dir = bool(getattr(args, "confirm_script_dir", False))
    scaffold_write_gate(confirm_script_dir, command="init")
    ...
    with file_lock() as guard:
        ...
        _companion_lines, _companion_apply_errors = apply_companions(
            _companion_plan, confirm_script_dir=confirm_script_dir)
        ...
        if "gitignore-block" in capability_ids or should_manage_gitignore(args):
            guard.require_owned()
            results.append(ensure_gitignore_block(confirm_script_dir))
        else:
            results.append(tr("gitignore_skipped"))
        results.extend(_companion_lines)
        results.extend(_runner_lines)
        guard.require_owned()
        results.extend(apply_init_capabilities(
            capability_ids, args.profile, name, full, confirm_script_dir))
...
def main():
  ...
  m8shift.py append agent-a --to agent-b --ask "review the change" --done "implemented it"''',
    ...
    i.add_argument("--confirm-script-dir", action="store_true",
                   help="explicitly allow init when the physical current directory differs "
                        "from the kit directory; writes still go beside m8shift.py")
    ...
    ci.add_argument("--companions", default="",
                    help="comma-separated companions to copy into the kit dir: "
                         "top,runtime,context,worktree,headroom,i18n,e2e")
    ci.add_argument("--with-top", action="store_true", help="copy m8shift-top.py")
    ...
    ci.add_argument("--full", action="store_true",
                    help="copy all operational companions (top,runtime,context,worktree,headroom,i18n)")
