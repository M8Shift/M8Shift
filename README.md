...
| 040 | [AI session usage monitoring](docs/en/rfc/040-rfc-ai-session-usage-monitoring.md) | Normalized local usage sidecars and per-agent admission holds | shipped phases + #88 amendment | v3.53–v3.54 · 2026-07-05 | runtime adapters/snapshot/guard; exact binding reset; no new global cooldown |
...
| 072 | [Exact-identity fleet bootstrap](docs/en/rfc/072-rfc-exact-identity-fleet-bootstrap.md) | Bootstrap listeners and schedule verified isolated jobs | implemented | v3.61.0 · 2026-07-16 | O1–O6; all 6 slices: `fleet plan/health/apply/reconcile/stop/resume` + immutable jobs with shell-free verification recipes, max-2 producer worktrees, integrator-only merge/handoff/drop |
| 073 | [Adapter registry and detached durability](docs/en/rfc/073-rfc-adapter-registry-detached-durability.md) | Deliver RFC 067 as adapter, durable-supervisor, live-provider, and routing slices | slices 1–3 + routing phase 1 implemented | v3.61.0 + target v3.63.0 · 2026-07-17 | live Gemini CLI 0.51.0 API-key/model-pinned fresh launch (native resume fail-closed), durable fleet control, and five-class advisory tier/effort matrix; no automatic routing |
