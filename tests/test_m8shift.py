...
class TestPureFunctions(unittest.TestCase):
    ...
    def test_usage_watch_incident_variants_detect_and_converge(self):
        ...
        # A live pre-lease watcher is killed before a managed lease is adopted.
        legacy = dict(base, lease_id="")
        legacy_result = runtime.usage_watch_reconcile_decision(
            legacy, now_utc=instant, pid_probe=lambda _pid: True)
        self.assertEqual((legacy_result["health"]["status"],
                          legacy_result["action"]), ("legacy", "recycle"))

        # A transient identity-probe failure for a live pid cannot authorize a
        # duplicate start, even when --replace was requested.
        unverifiable = dict(base, lease_id="lease-a",
                            process_start_ref="process-start-a")
        with mock.patch.object(runtime, "fleet_process_start_ref", return_value=""):
            invalid = runtime.usage_watch_start_decision(
                unverifiable, replace=True, pid_probe=lambda _pid: True)
        self.assertEqual((invalid["allowed"], invalid["action"]),
                         (False, "investigate"))

        # Operator stop intent survives reconciliation.
        stopped = dict(base, phase="stopped", desired_state="stopped",
                       lease_id="", pid=None)
        stopped_result = runtime.usage_watch_reconcile_decision(stopped)
        self.assertEqual(stopped_result["action"], "keep_stopped")

    def test_usage_watch_legacy_identity_fails_closed_before_signal(self):
        path = os.path.join(REPO, "m8shift-runtime.py")
        spec = importlib.util.spec_from_file_location("m8shift_runtime_identity_test", path)
        runtime = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runtime)
        with mock.patch.object(runtime, "process_argv",
                               return_value=["/bin/sleep", "60"]), \
                mock.patch.object(runtime, "_posix_signal_group_or_pid") as send:
            self.assertFalse(runtime.usage_watch_terminate(4242, ""))
        send.assert_not_called()

    def test_usage_watch_stale_lock_race_has_one_winner(self):
        """Two stale breakers cannot both return ownership of different inodes."""
        path = os.path.join(REPO, "m8shift-runtime.py")
        spec = importlib.util.spec_from_file_location("m8shift_runtime_lock_test", path)
        runtime = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runtime)
        with tempfile.TemporaryDirectory(prefix="m8shift-watch-lock-") as directory:
            runtime.USAGE_WATCHERS_DIR = directory
            lock_path = runtime.usage_watch_lock_path("claude")
            with open(lock_path, "w", encoding="utf-8") as fh:
                json.dump({"pid": 999999, "token": "stale"}, fh)

            real_open, real_unlink = os.open, os.unlink
            observed = 0
            unlinks = 0
            guard = threading.Lock()
            both_observed = threading.Event()
            fresh_created = threading.Event()
            second_unlinked = threading.Event()
                ...
                try:
                    ...
                    if target == lock_path:
                        with guard:
                            ...
                            if observed == 2:
                                both_observed.set()
                        self.assertTrue(both_observed.wait(2))
                    raise
                if target == lock_path and observed >= 2 and not fresh_created.is_set():
                    fresh_created.set()
                    self.assertTrue(second_unlinked.wait(2))
                return fd
            ...
            def racing_unlink(target, *args, **kwargs):
                ...
                if ordinal == 1:
                    return real_unlink(target, *args, **kwargs)
                if ordinal == 2:
                    self.assertTrue(fresh_created.wait(2))
                    try:
                        return real_unlink(target, *args, **kwargs)
                    finally:
                        second_unlinked.set()
            ...
            def contender():
                try:
                    token = runtime.acquire_usage_watch_lock("claude")
                    successes.append(token)
                    release.wait(3)
                    runtime.release_usage_watch_lock("claude", token)
                except ValueError as exc:
                    failures.append(str(exc))

            with mock.patch.object(runtime.os, "open", side_effect=racing_open), \
                    mock.patch.object(runtime.os, "unlink", side_effect=racing_unlink), \
                    mock.patch.object(runtime, "fleet_process_start_ref",
                                      side_effect=lambda pid: "ref-%s" % pid):
    ...
    def test_usage_watch_lock_busy_is_reported_without_traceback(self):
        path = os.path.join(REPO, "m8shift-runtime.py")
        spec = importlib.util.spec_from_file_location("m8shift_runtime_busy_test", path)
        runtime = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runtime)
        stop_args = argparse.Namespace(json=True)
        reconcile_args = argparse.Namespace(json=True, warn_threshold=0.8,
                                             limit_threshold=1.0,
                                             stale_after_minutes=30)
        with mock.patch.object(runtime, "acquire_usage_watch_lock",
                               side_effect=ValueError("transaction is busy")), \
                contextlib.redirect_stderr(io.StringIO()), \
                contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(runtime.cmd_usage_watch_stop(stop_args, "claude"),
                             runtime.USAGE_GUARD_EXIT_ERROR)
            self.assertEqual(runtime.cmd_usage_watch_reconcile(
                reconcile_args, "claude"), runtime.USAGE_GUARD_EXIT_ERROR)

    def test_usage_watch_empty_adapter_set_is_not_a_failed_read(self):
        path = os.path.join(REPO, "m8shift-runtime.py")
        spec = importlib.util.spec_from_file_location("m8shift_runtime_empty_test", path)
        runtime = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runtime)
        payload = {"ok": True, "snapshots": [], "findings": [{
            "severity": "warning", "check": "usage.no_adapter",
            "message": "no enabled usage adapter matched",
        }]}
        args = argparse.Namespace(tick_timeout=1.0, interval=60,
                                  warn_threshold=0.8, limit_threshold=1.0)
        with mock.patch.object(runtime, "run_usage_watch_probe",
                               return_value=(payload, "")):
            success, findings = runtime.usage_watch_tick_probe(args, "claude")
        self.assertTrue(success)
        self.assertEqual(findings[0]["check"], "usage.no_adapter")
...
class TestRuntimeCompanion(CLIBase):
    ...
    def test_usage_watch_singleton_refuses_duplicate_and_stop_manages_registry(self):
        ...
        try:
            ...
            stopped = self.rt("usage", "watch", "stop", "--agent", "claude",
                              "--json")
            runtime = self.load_runtime()
            identity_visible = bool(row.get("process_start_ref")
                                    or runtime.usage_watch_process_matches(proc.pid))
            if identity_visible:
                self.assertEqual(stopped.returncode, 0,
                                 stopped.stdout + stopped.stderr + repr(row))
                proc.communicate(timeout=5)
            else:
                # Sandboxed macOS may deny both ps identity probes. Fail closed,
                # then prove an already-dead watcher can still be marked stopped.
                self.assertEqual(stopped.returncode, 30,
                                 stopped.stdout + stopped.stderr + repr(row))
                proc.kill()
                proc.communicate(timeout=5)
                stopped = self.rt("usage", "watch", "stop", "--agent", "claude",
            ...
            self.assertEqual(row["desired_state"], "stopped")

            reconciled = self.rt("usage", "watch", "reconcile", "--agent",
                                 "claude", "--json")
            self.assertEqual(reconciled.returncode, 0,
                             reconciled.stdout + reconciled.stderr)
            watcher = json.loads(reconciled.stdout)["watchers"][0]
            self.assertEqual(watcher["action"], "stopped")
    ...
    def test_usage_watch_reconcile_quarantines_malformed_registry(self):
        self.init()
        directory = os.path.join(self.d, ".m8shift", "runtime", "usage-watchers")
        os.makedirs(directory, exist_ok=True)
        path = os.path.join(directory, "claude.json")
        with open(path, "w", encoding="utf-8") as fh:
            fh.write("{broken")
        result = self.rt("usage", "watch", "reconcile", "--agent", "claude",
                         "--json")
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        payload = json.loads(result.stdout)
        self.assertEqual(payload["watchers"][0]["action"], "quarantined")
        with open(path, encoding="utf-8") as fh:
            row = json.load(fh)
        self.assertEqual((row["phase"], row["desired_state"]),
                         ("stopped", "stopped"))
        quarantines = [name for name in os.listdir(directory)
                       if name.startswith("claude.json.malformed-")]
