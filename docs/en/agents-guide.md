...
machine-global guidance: keep-listening habits, TTL workarounds, install gotchas,
"always do X before Y" rules. Each memorized workaround **masks a product gap** —
the agent stops feeling the friction a fresh user hits, so the bug never gets
filed. This section defines the recurring audit that converts those private
workarounds back into product issues. It is a **contributor-layer process only**:
no core behavior, no runtime state machine, nothing generated by `init`.

### Cadence

Run the audit **at each minor release** (alongside the release checklist), or
**monthly** when no minor release ships in the month. One auditor (any operating
agent, or the maintainer) sweeps every persistent-memory entry and machine-global
guidance rule that mentions M8Shift.

### Taxonomy — classify every entry

| class | definition | action |
|-------|------------|--------|
| **keep** | Genuine operator preference or environment fact the product cannot and should not absorb (e.g. a machine-local path, a personal workflow choice). | Leave in memory. |
| **mixed** | Part preference, part compensation for a product gap (e.g. "always run X before Y" where the product could hint or check). | Extract the compensating part into an issue; keep the preference part. |
| **parasite** | Pure workaround for a missing or weak product surface — a fresh user without this memory would be hurt (e.g. a memorized guardrail the generated guidance never states). | Convert into a product issue, then rewrite the memory entry to point at the shipped surface once it lands. |

### Local-only rule (hard)

Audit artifacts (the entry inventory, classifications, working notes) **carry
local paths, machine names, and personal context — they are NEVER committed** to
any M8Shift repository, issue, or relay file. Only the scrubbed product issues
derived from them are filed.

### Scrub checklist — before filing any derived issue

- [ ] No absolute or home-relative local paths (no `/Users/...`, `~/...`, drive letters).
- [ ] No machine names, account names, personal names, or e-mail addresses.
- [ ] No verbatim memory-entry text that embeds project-external context; restate the *product-facing* symptom instead.
- [ ] Precedents referenced by **date and issue number only**.
- [ ] The issue stands alone: a reader with zero access to the auditor's memory can act on it.

### Conversion template (symptom → masked product gap → issue)

```markdown
## Symptom (as memorized)
<what the agent's memory rule compensates for, restated product-facing>

## Masked product gap
<what a fresh user hits without this memory; which surface should carry it
 (protocol core / pack / reference / doctor advisory / docs)>

## Proposed product change
<smallest shipped surface that makes the memory entry unnecessary>

## Acceptance
<how we know the workaround can be deleted from agent memory>
```

### Precedent

The first audit (2026-07-04) swept the operating agents' persistent memory and
produced issues **#17–#25**, including the compression/raw-proof contract (#22),
the shared-checkout destructive-git guardrails (#23), and this process itself
(#25). Reference it as "the 2026-07-04 audit" — by date and issue numbers only.
