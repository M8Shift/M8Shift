...
flowchart TB
    ...
    HD["🧠 Headroom-compatible command — optional external transform<br/>argv-only · one-shot · SHA-256 pinned<br/>explicit / config opt-in broad-context compression"]
...
an adapter-compatible Headroom command is present, identity-pinned, and explicitly selected (or
enabled through `backends.headroom_ext.auto_enabled`), it may run `headroom_ext` as an argv-only
broad-context transform. **`m8shift-worktree.py`**
