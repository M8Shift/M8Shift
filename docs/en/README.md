...
| [042-rfc-compression-backend-routing.md](rfc/042-rfc-compression-backend-routing.md) | Draft hybrid builtin/Headroom compression routing — pick the backend by the real determinant (retrieval-available + query-shape, not content-type): retrieve-capable+few-facts → builtin, no-retrieval/whole-content → Headroom. Fail-closed to builtin; **auto-route-to-Headroom gated behind measured evidence (#84) + identity-pinning**. Same evidence-gated philosophy as RFC 039, applied to the compressor. |
