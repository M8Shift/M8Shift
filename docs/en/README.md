...
| [039-rfc-model-task-routing.md](rfc/039-rfc-model-task-routing.md) | Draft advisory model/task-type cost-routing companion — a task-type taxonomy with capability floors, a provider-neutral axes+tiers comparison framework (operator manifest, no baked-in prices), skill min/optimum annotations, and a runtime `route recommend \| delegate` verb reusing RFC 028/020/014 launch machinery; capability-first with cost as the tie-breaker, advisory (no hidden auto-scoring), answers RFC 032's open questions. |
