...
| [040-rfc-ai-session-usage-monitoring.md](rfc/040-rfc-ai-session-usage-monitoring.md) | Draft usage-monitoring companion: normalized provider usage snapshots with provenance, a core `cooldown` command for IDLE/AWAITING relays, cooperative interrupts for a WORKING holder, and quiet waits until the provider usage window resets. Generic multi-provider — Claude + Codex phase-1 adapters, Gemini/Copilot/Vibe later; adapter security reuses the RFC 034 hardened runner. |
