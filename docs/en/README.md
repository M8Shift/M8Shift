...
| [037-rfc-agent-context-compression-backends.md](rfc/037-rfc-agent-context-compression-backends.md) | Draft context-compression policy + JSON file protocol — measure before send, digests/bounded references instead of raw bulk, redacted local raw stores, fail-closed to reference-only. Backends are RFC 034 adapters (builtin/RTK/optional Headroom); mechanism under RFC 033 policy; estimates advisory, gains validated with real tokenizers; compression never starves verification. |
