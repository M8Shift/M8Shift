...
| [037-rfc-agent-context-compression-backends.md](rfc/037-rfc-agent-context-compression-backends.md) | Context-compression policy + JSON file protocol — Phase C shipped in v3.38.0 with local `compress/retrieve`, builtin digesting, RTK backend dispatch for shell/tool outputs, redaction-before-store, bounded retrieval, and reference-only fail-safe. Backends are RFC 034 adapters (builtin/RTK/optional Headroom); mechanism under RFC 033 policy; estimates advisory, gains validated with real tokenizers; compression never starves verification. |
