...
`headroom` command. RFC 042 Phase B additionally records `--access-mode` and `--whole-content`
signals for the later measured gate; those signals do not drive routing yet.
...
`auto` default, retaining manual Headroom opt-in, and gating signal-driven routing until RFC 042's
evidence gate opens.
