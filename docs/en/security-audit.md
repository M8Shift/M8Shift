...
| Runtime sidecars are advisory | `m8shift-runtime.py`, `docs/en/rfc/009-rfc-runtime-companion.md` | Presence, inbox, idempotency, and progress files never grant the pen and never edit `M8SHIFT.md` directly. |
...
| Provider argv rendering avoids shells | `m8shift-runtime.py providers render`, `docs/en/rfc/014-rfc-provider-management.md` | Provider commands are JSON argv arrays with explicit placeholder substitution, not shell strings. |
| Headless child timeout is bounded | `examples/headless_runner.py`, `docs/en/rfc/020-rfc-headless-runner-hardening.md` | A stuck provider process can be terminated/killed and audited without stealing the pen. |
