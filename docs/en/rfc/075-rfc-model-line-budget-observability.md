...
retrieval was unavailable during this turn; claims that depend on product UI or
undocumented CLI RPC are labelled accordingly rather than promoted to fact.

## 3. Comparative matrix

| Vendor / surface | Observable granularity | Freshness | Access cost | Authentication | Can warn for one model line? |
|---|---|---|---|---|---|
| Anthropic Messages API rate-limit response headers | Organization/workspace limit applying to the requested model class; shared classes remain possible | Per response | Low once traffic is already instrumented | API key | **Yes, for API traffic**, at the documented model-class bucket; not for an unrelated Claude subscription |
...
| Mistral API 429/error surface | Organization/workspace rate-limit refusal; no stable per-line remaining/reset contract established in the reviewed evidence | On rejection | Low | API key | **Diagnostic only**; model-line anticipation remains unknown |
| Mistral La Plateforme limits/usage/billing console | Organization/workspace limits and usage/billing views | Operator refresh/reporting cadence | Medium/manual | Signed-in workspace admin | **Unknown/console-only**: do not assume a spend-by-model view is an enforcement bucket |
...
organization/workspace level and separates model classes; a caller must preserve
the class actually governing the request. This is the strongest near-live
line-level signal in API mode.

The Admin Usage and Cost API adds model-attributed history with privileged
organization access. It can explain burn and support forecasting, but a usage
bucket is not a promise of remaining capacity unless joined to the corresponding
limit. The Claude Code subscription surfaces already normalized by RFC 040 expose
account windows, not a documented model-line key. They must remain aggregate.
...
The reviewed official evidence establishes organization/workspace rate limiting,
429 failures, and authenticated console limit/usage surfaces. It does not
establish a supported machine-readable remaining/reset contract keyed by model
line for either La Plateforme subscriptions or Vibe. Phase 1 therefore records
Mistral line headroom as unknown. Historical usage or a model name in an error is
not promoted into a proactive quota signal.
...
- API mode can obtain strong per-request bucket evidence from Anthropic and
  OpenAI, model-dimensioned cloud quota evidence from Google, and only
  rejection/console evidence currently established for Mistral.
...
- Mistral usage and billing administration:
  <https://docs.mistral.ai/deployment/laplateforme/administration/>

URLs identify the intended official evidence. Because live retrieval was not
available for this draft, Phase 2 review must re-open each contract and correct
any moved page or changed field before implementation.
