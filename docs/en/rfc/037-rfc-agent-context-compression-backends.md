...
- Status: draft; Phase C local records + builtin/RTK backend dispatch shipped in v3.38.0; Phase D optional Headroom adapter dispatch shipped in v3.39.0; Headroom manual auto opt-in gate shipped in v3.40.0; RFC 042 Phase B records future signal-driven routing hints without changing that manual opt-in
...
automatic broad-context Headroom experiments. RFC 042 Phase B records `access_mode` /
`whole_content` routing signals but does not let those signals drive Headroom selection before the
measured gate opens. Explicit `--backend headroom_ext` does not require that opt-in.
