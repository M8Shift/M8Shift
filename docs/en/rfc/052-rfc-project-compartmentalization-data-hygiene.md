...
- **PR 4** *(design rev 2 — RFC 038 §9 amendment, post Codex design review: centralized pre-write gate for ALL mutators under the file lock, deterministic bind target with explicit selector, live-pen guard on rebind, samefile identity, one basename+hash disclosure rule, sessions-list deferred in favor of bind --list)*: session-binding mechanics — A1 write-refusal on two-candidate relay ambiguity (leftover `M8SHIFT_ROOT` + cwd-local relay = the recorded leak vector), A2 penless per-agent `bind` record, A3 fail-closed binding verification in `may-i-write`/`claim`/`append` (foreign path redacted), A4 read-only `sessions list` (RFC 038 surface, single-project), A5 agent-pack mechanical line + floor marker. Zero-config path byte-identical.
