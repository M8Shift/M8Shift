...
- **PR 4** *(implemented — RFC 038 §9 amendment, design rev 3 approved by Codex)*: session-binding mechanics. Candidates = the `M8SHIFT_ROOT` relay vs the **script-local (HERE)** relay only (no cwd walk, no sibling scan); physical identity via realpath + `samefile` (normcase fallback on Windows, never unconditional casefold). **A1** = ONE centralized pre-write ambiguity gate for EVERY mutator — agentless admin writes (`archive`, `cooldown`, `decisions --set`, `session report --write`) always refuse under unresolved ambiguity; refusal precedes any lock-file creation; `update --target` keeps its explicit authority; `init` refuses any non-empty `M8SHIFT_ROOT` resolving to a different physical root than HERE (even if not yet existing). **A3** = per-agent binding verification on actor-bearing mutators only, re-checked under the relay file lock (TOCTOU); `may-i-write` reports binding validity read-only. **A2** = penless `bind` with deterministic target (closed `--candidate env|script` selector under ambiguity, never silent env-wins) + live-pen guard (refuses while the agent holds a live WORKING lock in any candidate). **Inventory** = read-only `bind --list` (`sessions list` DEFERRED — `session list` homonym trap). ONE disclosure rule everywhere: foreign roots as basename + sha256-10. Zero-config path byte-identical.
