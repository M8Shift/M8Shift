...
- **PR 2**: C3 out-of-repo confidential denylist + `scripts/scrub-check.py` (tip + history) + optional pre-push hook + tests (argument-order safety, RTK-bypass, denylist redaction/precedence).
- **PR 3**: optional anchor hygiene (explicit allowed roots, advisory, no default rc 1). After PR 1/2 stable.
- **PR 4**: session-binding mechanics (RFC 038 amendment — `sessions list`/bind, ambiguity refusal). Scope TBD.
