...
- **Scope:** external security research (arXiv agentic-security defenses), MITRE ATLAS,
  ...
  (`m8shift.py` v3.45.0 and its companions).
...
| Incomplete AI agent evaluation (11) / Lack of testing diversity (86) | Agentic + Non-Technical / robustness | **APPLICABLE (to M8Shift's own dev)** | Applies to M8Shift-as-software, not as a model. Lead: the existing 400-test suite (as of v3.45.0 — regenerate on release; a test *count* is not evidence of coverage or diversity, so risk 86 is addressed by adversarial + invariant/property tests, not a headline number) plus adversarial dogfooding (Codex implements / Claude reviews) is the mitigation. |
...
- **Incomplete AI agent evaluation (11) / Lack of testing diversity (86) / Lack of system
  ...
  schema validation, the 400-test suite (count ≠ coverage), and adversarial dogfooding.
...
| **MEASURE** | Quantitative/qualitative *model* eval: metrics, bias, robustness, drift | **NOT-APPLICABLE (largely)** | No model ⇒ no accuracy/bias/robustness metrics. Inspiration-only carve-out: `doctor` diagnostics + the 400-test suite are *software-quality* measurement, not "AI measurement." Do **not** invent model metrics. |
...
| **R24** Tests fonctionnels avant déploiement | Functional tests | **APPLICABLE — already satisfied** | **400 tests** (v3.45.0, regenerate on release), VERSION lockstep across 7 scripts. |
...
| **Four formal security properties** ([2603.19469](https://arxiv.org/abs/2603.19469)) | roster + turn schema + lane scope; 400-test suite | **APPLICABLE** | Encode task/action alignment, source-authorization, data-isolation as asserted unit-test invariants. |
...
defense papers. Verified code `m8shift.py` v3.45.0.*
