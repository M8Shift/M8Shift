...
- **Scope:** external security research (arXiv agentic-security defenses), MITRE ATLAS,
  ...
  (`m8shift.py` v3.26.0 and its companions).
...
| Incomplete AI agent evaluation (11) / Lack of testing diversity (86) | Agentic + Non-Technical / robustness | **APPLICABLE (to M8Shift's own dev)** | Applies to M8Shift-as-software, not as a model. Lead: the existing 255-test suite + adversarial dogfooding (Codex implements / Claude reviews) is the mitigation. |
...
- **Incomplete AI agent evaluation (11) / Lack of testing diversity (86) / Lack of system
  ...
  schema validation, the 255-test suite, and adversarial dogfooding.
...
| **MEASURE** | Quantitative/qualitative *model* eval: metrics, bias, robustness, drift | **NOT-APPLICABLE (largely)** | No model ⇒ no accuracy/bias/robustness metrics. Inspiration-only carve-out: `doctor` diagnostics + 255-test suite are *software-quality* measurement, not "AI measurement." Do **not** invent model metrics. |
...
| **2. Secure development** — "Secure your supply chain" | Vet/secure dependencies and build inputs | **APPLICABLE — strongly satisfied** | **Stdlib-only, zero third-party deps** = minimal surface by construction; single file; installers verify against `checksums.sha256` (pinned `--sha256`/signed tags). M8Shift's single biggest CISA-alignment strength. |
...
| **R24** Tests fonctionnels avant déploiement | Functional tests | **APPLICABLE — already satisfied** | **255 tests**, VERSION lockstep across 7 scripts. |
...
| **Four formal security properties** ([2603.19469](https://arxiv.org/abs/2603.19469)) | roster + turn schema + lane scope; 255-test suite | **APPLICABLE** | Encode task/action alignment, source-authorization, data-isolation as asserted unit-test invariants. |
...
defense papers. Verified code `m8shift.py` v3.26.0.*
