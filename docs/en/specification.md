...
| EF-19b | The generated `commit-msg` hook stamps dogfooding provenance fail-open: `Coordinated-With: M8Shift vX.Y.Z` from the active relay, and `Agent-Model: <id>` when `M8SHIFT_AGENT_MODEL` is present and matches the safe single-line model-id charset. Existing trailers are never duplicated, malformed model ids are ignored without blocking the commit, and verbose commit messages keep trailers above the scissors line. | `test_commit_msg_hook_injects_coordinated_with_from_m8shift_root`, `test_commit_msg_hook_stamps_agent_model_when_declared`, `test_commit_msg_hook_skips_absent_agent_model`, `test_commit_msg_hook_rejects_malformed_agent_model_fail_open`, `test_commit_msg_hook_keeps_existing_agent_model_idempotent`, `test_commit_msg_hook_keeps_trailer_in_body_for_verbose_commit` |
| EF-20 | `examples/headless_runner.py` is a reference local runner for one headless agent lane: it waits/claims through the core, writes an immutable `.m8shift/runtime/run-plans/<run-id>.json` plan for one static argv command, refreshes the TTL heartbeat before expiry, passes `M8SHIFT_RUN_ID`, verifies the post-run `LOCK` state against the plan, and appends lifecycle/finding events to `.m8shift/runtime/runs.jsonl`. | `test_headless_runner_once_writes_run_ledger_and_env_run_id`, `test_headless_runner_reads_m8shift_lock`, `test_headless_runner_run_plan_is_immutable`, `test_headless_runner_post_run_verification_detects_mismatch` |
...
| [009-rfc-runtime-companion.md](rfc/009-rfc-runtime-companion.md) | **Runtime companion v1** | `m8shift-runtime.py watch/operator/progress/status-runtime/doctor/retention prune`; `.m8shift/runtime/{presence.json,runs.jsonl,progress.jsonl,idempotency.jsonl,approvals.jsonl,inbox/*.jsonl,archive/*.jsonl}` with `m8shift.runtime.event.v1` rows, per-agent lane ownership, no-progress findings, stale-presence diagnostics, composed `status-runtime --brief`, last run events, and basic fixed-count JSONL pruning. | Local advisory sidecars only; invalid/deleted sidecars are diagnostic-only; stale lane takeover is explicit and never grants the core pen; no-progress blocks only the companion loop; retention never edits `M8SHIFT.md` and leaves malformed JSONL untouched; no direct `M8SHIFT.md` edits and no second pen authority. |
