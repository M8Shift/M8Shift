...
| EF-20 | `examples/headless_runner.py` is a reference local runner for one headless agent lane: it waits/claims through the core, writes an immutable `.m8shift/runtime/run-plans/<run-id>.json` plan for one static argv command, validates the mandatory RFC 028 run-plan fields (`agent`, `argv`, `cwd`, `run_id`, `prompt_hash`, `env_allowlist`, `timeout`, `kill_grace`, `expected_transition`), refreshes the TTL heartbeat before expiry, passes only an explicit env allowlist plus `M8SHIFT_*`, verifies the post-run `LOCK` state against the plan, and appends lifecycle/finding events to `.m8shift/runtime/runs.jsonl`. A run is successful only if exit code is `0`, the expected transition occurred, the lock was not stolen, and ledger events were recorded. | `test_headless_runner_once_writes_run_ledger_and_env_run_id`, `test_headless_runner_reads_m8shift_lock`, `test_headless_runner_run_plan_is_immutable`, `test_headless_runner_validates_run_plan_fields_and_argv`, `test_headless_runner_post_run_verification_detects_mismatch`, `test_headless_runner_post_run_validation_rejects_stolen_lock_and_nonzero_exit` |
...
| EF-26 | `m8shift-context.py adapters init/list/show/check/run` implements the Phase-2 companion adapter boundary: manifests are JSON, commands are argv arrays, `command[0]` is a bare allowlisted program resolved through `PATH`, the resolved executable must match the manifest's trusted absolute path + SHA-256 identity, stdout/stderr/runtime are bounded, environment is allowlisted, failures follow the manifest policy, and adapter output is advisory only. The shipped RTK shell-output manifest recommends `err`/`test`/`log`/`ls` and forbids `git-diff` for code review because measured output is lossy for hunks. Since v3.34.0, `pack` defaults to RTK only when `rtk` is present and identity-pinned; absent/unpinned RTK degrades to native packing, `--adapter native` / `--no-rtk` opts out, setup attempts `rtk telemetry disable`, and `doctor --json` surfaces RTK presence, pin status, and telemetry state. Since v3.34.1, corrupt, unreadable, or non-object RTK manifests also degrade to native in automatic mode, while explicit `--adapter rtk-shell-output` remains fail-closed. | `TestContextAdapters`, `test_pack_defaults_to_pinned_rtk_when_present_else_native_and_opt_out`, `test_pack_auto_degrades_corrupt_rtk_manifest_to_native`, `test_init_disables_rtk_telemetry_and_doctor_surfaces_state` |
...
core parse failure and is not mutated by retention. Deleting
...
| [009-rfc-runtime-companion.md](rfc/009-rfc-runtime-companion.md) / [026-rfc-sidecar-retention.md](rfc/026-rfc-sidecar-retention.md) / [027-rfc-notifications.md](rfc/027-rfc-notifications.md) | **Runtime companion v1** | `m8shift-runtime.py watch/operator/progress/status-runtime/doctor/notify/retention prune/retention apply/retention policy show`; `.m8shift/runtime/{presence.json,runs.jsonl,progress.jsonl,idempotency.jsonl,approvals.jsonl,inbox/*.jsonl,notify.config.json,notify/*,archive/*.jsonl,retention.json}` with `m8shift.runtime.event.v1` rows, per-agent lane ownership, no-progress findings, stale-presence diagnostics, local notification tiers, composed `status-runtime --brief`, last run events, basic fixed-count JSONL pruning, and opt-in manifest retention strategies (`fixed-count`, `age`, `combined`). | Local advisory sidecars only; invalid/deleted sidecars are diagnostic-only; stale lane takeover is explicit and never grants the core pen; notifications never mutate `M8SHIFT.md`, never daemonize, never use the network, and hooks/OS commands are argv-only best-effort side effects; no-progress blocks only the companion loop; retention never edits `M8SHIFT.md`, leaves malformed JSONL untouched, keeps undatable rows for age-based policies, and writes archive audit rows only for opted-in runtime ledgers; no direct `M8SHIFT.md` edits and no second pen authority. |
