...
| EF-7 | The holder can refresh its own lock TTL. Automated wrappers/runners MUST refresh the TTL with `claim <me> --refresh` (an audit-only beat; PROTECTIVE liveness is the `heartbeat` verb — RFC 049) (refused unless already holding the own `WORKING` lock — a plain claim can ghost-claim a fresh turn after a race, RFC 047); plain `claim <me>` remains only the legacy/manual self-claim path while already holding the pen. Heartbeat at least 5 minutes before expiry. | `test_reclaim_own_lock_refreshes`, `TestRFC047PhaseA.test_t8_claim_refresh_guard` |
