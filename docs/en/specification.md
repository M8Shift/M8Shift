...
| EF-10d | `update --target DIR [--source DIR]` updates an initialized target project from the running source copy. It writes protocol/pack/anchors/installed companions before replacing the core, never copies relay/session state from the source, refuses unsupported baselines, downgrades, checksum failures, source/driver version splits, symlink/path escapes, and active `WORKING_*` targets by default, and records bounded update audit rows when it writes. | `TestRFC048PRB` |
