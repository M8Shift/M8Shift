...
| EF-33 | **Usage monitoring surface (RFC 040 Phases 3–4 + RFC 051)**: `m8shift-runtime.py usage init/adapters/snapshot/status/guard/watch/wait/resume` manages a disabled-by-default adapter registry (`.m8shift/usage/adapters.json`), normalizes adapter output to append-only `m8shift.usage.snapshot.v1` ledger events, and evaluates an advisory guard whose `--apply` parks the relay ONLY through the core `cooldown` command (`usage resume` is the only road back — never automatic). The reference adapters (`examples/usage-adapters/claude-oauth-usage.py`, `examples/usage-adapters/codex-ratelimits.py`, and the spend-only `examples/usage-adapters/tokscale-spend.py` — #103: bounded tri-state token reader with explicit recognized-state so a known zero is data, `local_estimate` provenance, no invented windows/limits, and a hard never-submit exact-token guard that refuses leaderboard verbs BEFORE any launch) are fail-open (empty official fixture on any provider/auth/schema error), never emit account identity/credits/plan/raw responses, and are pinned to the live provider shapes (Claude: top-level `five_hour`/`seven_day` used-percent + strict-`Z` reset normalization with naive → `null`; Codex: held-open-stdin JSON-RPC with a daemon reader thread and a monotonic deadline). Core `status`/`watch` render the RFC 051 read-only advisory usage line — including amendment E (#106), the unified multi-window form — by echoing recorded sidecar bytes: never computing, never running an adapter, byte-identical output when no usable snapshot exists. | `TestRFC040UsagePRA`, `TestRFC040UsagePRB`, `TestRFC040UsageQuota`, `TestRFC040UsageBudget`, `TestRFC040CodexRateLimitsAdapter`, `TestRFC040TokscaleSpendAdapter`, `TestRFC051UsageAdvisory` |
...
| [051-rfc-usage-advisory-in-core-display.md](rfc/051-rfc-usage-advisory-in-core-display.md) | **Usage advisory in the core display** — implemented v3.54.0 line; #59 consumption fragment; amendment E #106 unified multi-window line | `status` / `watch` (and `scripts/watch-status.sh`) render a read-only advisory usage line per roster agent from the freshest recorded `usage.snapshot` event in `.m8shift/runtime/usage.jsonl`: recorded percent + window + verbatim `provenance` + staleness marker, with the #59 bounded consumption fragment. Amendment E (#106): when a snapshot carries at least one additional usable window, the line unifies all recorded windows (consumed percentage and reset per window, deterministic max-ratio tie rule, field-level degradation, a hard cap against hostile many-window payloads, and fallback to the single-window form when `windows[]` is unusable). | Bounded path-safe tail read (`lstat` first, tail never trusts a partial first line); echo-recorded-bytes only — the core never computes ratios or argmax and never runs an adapter; roster-only rendering; terminal/output-safe fields; byte-identical human and `--json` output when no usable snapshot exists. |
