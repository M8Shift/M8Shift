...
| EF-29 | `m8shift-context.py compress` creates local RFC 037 context-compression records without touching the core relay: each `compressed_context_record` wraps `m8shift.adapter.result.v1`, includes `context_digest`, `handoff_digest`, and `raw_output_reference`, stores redacted raw evidence before any compact digest, and uses the builtin stdlib compressor for head/tail, repeated-line collapse, error/path extraction, and exit-code preservation. Missing/malformed compression config and backend failures fail closed to redacted reference-only. `retrieve` resolves record ids only through `SAFE_ID_RE` + confined paths, always returns bounded output, and uses native stdlib `re` grep with pattern-size and ReDoS guards. | `TestContextCompression` |
