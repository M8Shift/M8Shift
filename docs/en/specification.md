...
| [048-rfc-adoption-discipline-pack-update-health.md](rfc/048-rfc-adoption-discipline-pack-update-health.md) | **Adoption discipline pack + health diagnostics** — PR A implemented v3.49.0 | `init` generates `M8SHIFT.agent-pack.md`; anchor stanzas keep a compact inline safety floor and point to the pack/protocol; `init --force-generated` repairs only corrupted generated pack blocks; `doctor` exposes adoption pack/stanza health and `doctor --json` includes a live adoption report. | Adoption aid only: the pack and diagnostics teach/verify the workflow but never grant write authority, never change claimability, and never repair unless the operator explicitly runs `init` / `init --force-generated`. |
