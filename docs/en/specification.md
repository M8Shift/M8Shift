...
| EF-10d | `update --target DIR [--source DIR]` updates an initialized target project from the running source copy. It writes protocol/pack/anchors/default installed runners/installed companions before replacing the core, never copies relay/session state from the source, refuses unsupported baselines, downgrades, checksum failures, source/driver version splits, symlink/path escapes, and active `WORKING_*` targets by default, and records bounded update audit rows when it writes. Before the first component mutation it also preflights the companion scaffold write gate, so a conflicting `$M8SHIFT_ROOT` refuses the whole update without a partial protocol/pack/anchor write. The default `runner` component refreshes only already-installed `scripts/watch-status.sh` / `examples/headless_runner.py` artifacts whose current checksum is proven by `.m8shift/kit.json` `runners[]`; absent runners are never created, and present-but-untracked regular runners are skipped in the default update path so full checkouts/dogfood trees do not fail. Explicit `--components runner` treats untracked runners as `manual_review_required`; edited tracked runners are also `manual_review_required`; non-regular, symlinked, or escaping runner targets are `refused`. A mixed companion or runner outcome (at least one refreshed/current AND at least one refused/manual-review) reports that component as `partial` with per-file outcomes carried in `--json` and the audit row; `partial` is not an OK result, so the run still reports incomplete and exits non-zero (#43/#60). `doctor --source` is read-only and emits `runner.stale` / `runner.manual_review_required` only for tracked or dangerous runner preflight findings; it does not warn for regular untracked runners skipped by default update. | `TestRFC048PRB`, `test_conflicting_m8shift_root_refuses_before_any_component_write` |
...
| EF-20 | `examples/headless_runner.py` is a reference local runner for one headless agent lane. Before launch, `--handshake` emits at most 4096 bytes of one-line `m8shift.runner.handshake.v1` JSON with `version`, bounded `capabilities` (`bounded-tty-tee-v1`, `environment-write-probe-v1`, `runner-exit-v2`), and supported `options`; it performs no relay or provider work. A normal run waits/claims through the core, writes an immutable `.m8shift/runtime/run-plans/<run-id>.json` plan for one static argv command, validates the mandatory RFC 028 run-plan fields (`agent`, `argv`, `cwd`, `run_id`, `prompt_hash`, `env_allowlist`, `timeout`, `kill_grace`, `expected_transition`), refreshes the TTL before expiry **via `claim <agent> --refresh` only** (audit-only beat; the supervising listener owns protective liveness via the `heartbeat` verb — RFC 049), passes only an explicit env allowlist plus `M8SHIFT_*`, and appends lifecycle/finding events to `.m8shift/runtime/runs.jsonl`. Provider output passes through a bounded TTY tee that records byte/line counts and allowlisted signature IDs, never raw output in state/notifications. Post-run classification is **authorship-primary and total**; an apparent environment refusal becomes terminal only after `environment-write-probe-v1` confirms the configured cwd is unwritable. Exit codes (`runner-exit-v2`): `0` success, `1` classified failure, `2` argparse refusal only, `3` external transition, `4` suspended, `5` infrastructure/timeout. Full success additionally requires provider exit `0` and recorded ledger events; no automatic force-claim exists on any path. | `TestRFC047PhaseA`, `TestListenerCompatibilityIncident`, `test_current_runner_exposes_the_bounded_capability_handshake`, `test_read_only_words_alone_remain_retryable_non_completion`, `test_confirmed_environment_block_halts_after_one_redacted_attempt` |
...
| EF-30 | `m8shift-runtime.py listener start/stop/status/logs` is the RFC 047 lifecycle companion that supervises one headless runner lane per agent. Before any non-dry-run pid/state/provider work, startup runs the bounded handshake probe and classifies the runner as `CURRENT`, `LEGACY`, `BROKEN`, or `ABSENT`; only current proceeds, legacy/broken/absent return a provisioning remedy, and dry-run records `not_probed` without executing the runner. The loop polls the `LOCK` read-only with **zero model spend** while it is not the agent's turn and wakes **exactly one bounded runner turn** on `AWAITING_<agent>` (or the single permitted `start_on_idle`). The listener owns retry counting over authoritative ledger classification first, then `runner-exit-v2`: 0 resets, bare 2 is terminal argparse refusal, 3/4 are neutral, and 1/5/unknown launch failures are retryable. A pre-existing ledger timeout or non-completion is never overwritten by an exit-code fallback. Every public surface consumes the core `listener_snapshot` table: lifecycle (`ALIVE`, `HALTED (resident)`, `UNKNOWN`), coverage (`invoker`, `notifier`, `halted`, `absent`, `unknown`), attention, and bounded cause vocabulary are one decision rather than independently inferred labels. The fold reads each sidecar once. Existing TTL refresh, heartbeat, own-stuck recovery, halt persistence, process-group control, service-backend, log-rotation, and advisory-doctor invariants remain unchanged. | `TestRFC047ListenerPR1`, `TestRFC047ListenerPR2`, `TestRFC049PRBListenerProducer`, `TestListenerCompatibilityIncident`, `test_listener_snapshot_decision_table`, `test_status_listener_snapshot_reads_each_sidecar_once` |
