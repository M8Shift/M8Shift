...
- `.m8shift/runtime/*` — `presence.json`, `runs.jsonl`, `consults.jsonl`, `progress.jsonl`, `approvals.jsonl`, `idempotency.jsonl`, `inbox/<agent>.jsonl`, `notify.config.json`, `notify/log.jsonl`, `usage-watchers/<agent>.json`, `retention.json`. Usage watcher records are managed singleton leases: `usage watch stop --agent` records a stopped phase and `usage watch reconcile [--agent]` repairs dead, stale-tick, or repeated-unknown readers without touching the relay or automatically clearing a usage hold.
