...
| `claim A [--force|--refresh] [--check --files --turns]` | local-state (`--check` read-only) | LOCK, turns | LOCK | exclusive acquire; `--force` reclaims a **stale** lock only; `--refresh` only **extends A's own `WORKING` lock** (refused otherwise, exclusive with `--force`) — runners use it for TTL extension (it records an audit-only beat), never a plain `claim` (RFC 047); PROTECTIVE liveness is the `heartbeat` verb (RFC 049) |
| `heartbeat A --source runtime-listener\|wrapper --cadence-seconds N` | local-state | LOCK (validated under `file_lock`) | `.m8shift/holder-heartbeats/<A>.json` | RFC 049 PROTECTIVE liveness beat: actor-bearing mutator (RFC 052-gated), allowed only while `A` holds its own `WORKING` lock, source/cadence validated (1..TTL seconds) before any write, protective window = max(120, min(2·cadence, TTL)); never extends `expires`, never claims/appends/releases |
