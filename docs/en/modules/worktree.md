...
- `.m8shift/worktree-owners/<id>.lock` — a transient per-id ownership lock (`O_CREAT\|O_EXCL` + `<pid>:<ns>` token) held across each id's ownership mutation (claim create, takeover, and the whole drop span) so those serialize without holding the global core lock across git. A stale lock is broken ONLY when its recorded holder PID is POSITIVELY proven dead and its inode/token is unchanged (a malformed/unknown token fails safe: never reclaimed, delete by hand after confirming no op is in flight), so a slow-but-live `git worktree add/remove` is never stolen; a directory/FIFO/non-regular lock is never reclaimed (the contender times out BUSY rather than spinning); the release unlinks only its own unchanged token; and a lock-acquisition failure in a SAFE owners directory FAILS CLOSED rather than silently running a mutation unserialized. Serialization is guaranteed only when the owners namespace is SAFE; an unsafe/uncontained namespace follows the documented advisory fail-open limitation (no ownership takeover mutation can occur there, but fail-open verbs such as a same-owner `drop` still do their normal work — the lock does not itself prevent a destructive operation under fail-open).
