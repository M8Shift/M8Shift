...
`branch` → PR (`Closes #<n>`) → independent review → merge when stable → push forge-first then GitHub.

## Decision log

<!-- Record decisions, agreements, and disagreements here as the task progresses;
     close with a short wrap-up (affected branches, merged PR, final outcome). -->
