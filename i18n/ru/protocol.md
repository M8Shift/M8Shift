...
> cowork рассчитан на репозиторий на локальном диске. См. также §0/§4 (обязательный claim).
...
```
./m8shift.py init [--name PROJECT] [--agents a,b] [--lang en|fr] [--force]  # (пере)создаёт набор здесь
...
cowork **пассивен**: он никогда не «вызывает» какой-либо ИИ. Он полагается на соглашение каждого
...
- **Общее ограничение**: cowork не может заставить ИИ что-либо прочитать. Без
