...
> cowork はローカルディスク上のリポジトリを対象としています。§0/§4（必須の claim）も
> 参照してください。
...
```
./m8shift.py init [--name PROJECT] [--agents a,b] [--lang en|fr] [--force]  # ここにキットを（再）生成
...
- **各アクティブなエージェントのアンカー**（デフォルトでは `CLAUDE.md` と `AGENTS.md`;
  ...
  保持; 以前のファイルは `<anchor>.cowork.bak` にバックアップ）;
...
cowork は**受動的**です: AI を「呼び出す」ことは決してありません。各ホストツールの規約に
...
- **一般的な制限**: cowork は AI に何かを読むよう強制できません。プロジェクトの
